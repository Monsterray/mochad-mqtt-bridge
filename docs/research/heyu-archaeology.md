...
## Prioritized Recommendations At Review Time
...
As of 2026-08-22, the bridge has independently implemented profile schema v2,
named-profile evidence gating, typed state confidence and provenance,
capability-aware global-command reconciliation, and a bounded declarative
command-sequence schema. These implementations were developed from project
contracts, manufacturer evidence, and original tests; this document did not
authorize source reuse from Heyu.

Sensor inactivity and low-battery handling, broader extended-command fixtures,
raw RF diagnostics, and security RF decoding remain research or future work.
They require separately reviewed specifications and the hardware evidence
listed below. This archaeology record is evidence for design decisions, not a
supported-device list or an implementation roadmap by itself.
