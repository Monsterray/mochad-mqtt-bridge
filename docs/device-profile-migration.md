...
- `candidate`: broader validation candidate, still requiring explicit opt-in;
...
Bridge configuration generated by this release includes:
...
Existing files without this field, or with an explicit value of `1`, are
migrated in memory before devices are parsed. The bridge does not rewrite an
existing file. Unknown versions fail startup rather than guessing. Schema v2
adds typed identity, capabilities, state policy, Home Assistant defaults,
global-command behavior, secondary channels, bounded non-executable sequences,
and evidence attached to individual model claims.
